...
class SearchAdapter(var items: List<Searchable>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder =
        when (viewType) {
            MOVIE_TYPE -> MovieGridAdapter.MovieViewHolder(
                MovieGridItemBinding.inflate(
                    LayoutInflater.from(parent.context)
                )
            )

            TV_TYPE -> TvGridAdapter.TvViewHolder(
                TvGridItemBinding.inflate(
                    LayoutInflater.from(
                        parent.context
                    )
                )
            ...
            PERSON_TYPE -> PersonGridAdapter.PersonViewHolder(
                PersonGridItemBinding.inflate(
                    LayoutInflater.from(parent.context)
                )
            )
